...
function AddProfileEducationalModal() {
  ...
  const [isPresentEducationPlace, setIsPresentEducationPlace] = useState(true);
  ...
  const AddEducationProfile = async (e) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/users/education`,
        {
          position: position,
          school: school,
          startDate: startDate,
          endDate: endDate,
          isPresentEducationPlace: isPresentEducationPlace,
          location: location,
          summary: summary,
  ...
  useEffect(() => {
    if (profile) {
      setPosition(profile.education.position);
      setSchool(profile.education.school);
      setStartDate(profile.education.startDate);
      setEndDate(profile.education.endDate);
      setIsPresentEducationPlace(profile.education.isPresentEducationPlace);
      setSummary(profile.education.summary);
      setLocation(profile.education.location);
    }
  }, [profile]);
...
export default AddProfileEducationalModal;
